...
import { createFighter as createSpark } from '../../src/fighters/spark.js';
import { createFighter as createPhantom } from '../../src/fighters/phantom.js';
...
describe('createFighter — name', () => {
  ...
  it('spark name', () => expect(createSpark(W, H).name.toLowerCase()).toContain('spark'));
  it('phantom name', () => expect(createPhantom(W, H).name.toLowerCase()).toContain('phantom'));
});

// ── 3. Uniform physics — all fighters have mass === 1 and size === 20 ──────
describe('createFighter — physics', () => {
  const factories = [createBlaze, createQuake, createSpark, createPhantom];
  const names = ['blaze', 'quake', 'spark', 'phantom'];
...
describe('1v1 balance — neither fighter wins >75%', () => {
  ...
  for (const [a, b] of matchups) {
    it(`${a} vs ${b}`, () => {
      ...
      if (a === 'spark' && b === 'phantom') hi = 0.80; // Tuner's best: 75.3%
...
describe('quake vs phantom balance', () => {
  it('quake wins between 30% and 70% of 500 simulations', () => {
    let quakeWins = 0;
    for (let i = 0; i < 500; i++) {
      const result = new BattleSimulation(`qvp-${i}`, ['quake', 'phantom']).runAll();
...
describe('BattleSimulation — no throw in 1v1', () => {
  for (const name of ['blaze', 'quake', 'spark', 'phantom']) {
